...
| `favicon.svg` | Modern browser favicon (gold-rimmed medallion, the Æ in the Bifröst gradient). |
| `favicon.ico` | Legacy fallback (16, 32, 48, 64 px embedded). |
| `apple-touch-icon.png` | iOS home-screen icon (180×180). |
| `icon-192.png` / `icon-512.png` | PWA / Android home-screen. |
| `stars.svg` | Cosmic starfield backdrop (generated by `render_stars.py`). |
| `fonts/` | Vendored logo face (Metamorphous) used by the render scripts. |
...
### Theme — Asgard / Viking future-tech

The look is the realm eternal: a carved **Metamorphous** wordmark in polished
Asgardian **gold** (`--gold`, `--gold-metal`), a cosmic starfield (`stars.svg`),
gold eyebrows and links, a slowly turning Elder-Futhark **rune ring** in the
hero, and the **Bifröst** — the rainbow bridge Heimdal guards — carried by the
Æ and split one-hue-per-app across the products.

- The Æ in the wordmark is `<span class="ae">æ</span>`, painted with `--bifrost`.
- Each app carries its own hue (`--c-*`), applied per card via an inline
  `--accent`. Maskin is blue, the house colour (`--signal`).
- Gold lives in `--gold` / `--gold-metal`; recolour those to retint the realm.
- `render_stars.py`, `render_og.py`, `render_favicons.py` regenerate the
  starfield, social image and icon family (Pillow required).

## Performance

- Single HTML file, ~26KB
- Four webfonts loaded from Google Fonts (Metamorphous for the wordmark, Space Grotesk for headings, JetBrains Mono for labels, Noto Sans Runic for the runes)
